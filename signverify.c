/* compile with
 * gcc -ldl signverify.c -o signverify
 */
...
int main(int argc, char **argv) {
	...
	fp->C_OpenSession(0, CKF_SERIAL_SESSION | CKF_RW_SESSION, NULL_PTR, NULL_PTR, &session_handle);

	fp->C_Login(session_handle, CKU_USER, "0001password", 12);
	...
	fp->C_SignInit(session_handle, &sign_mechanism, object_handle);
	fp->C_Sign(session_handle, message, message_length, sign, &sign_length);
	...
	fp->C_Verify(session_handle, message, message_length, sign, sign_length);
